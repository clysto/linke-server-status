...
install -D -v -m 755 node_exporter /usr/local/bin/node_exporter
install -D -v -m 644 node_exporter.service /usr/local/lib/systemd/system/node_exporter.service
...
if [ "$install_nvidia_gpu_exporter" = "y" ]; then
    install -D -v -m 755 nvidia_gpu_exporter /usr/local/bin/nvidia_gpu_exporter
    if [ ! -f /usr/local/lib/systemd/system/nvidia_gpu_exporter.service ]; then
        install -D -v -m 644 nvidia_gpu_exporter.service /usr/local/lib/systemd/system/nvidia_gpu_exporter.service
    fi
